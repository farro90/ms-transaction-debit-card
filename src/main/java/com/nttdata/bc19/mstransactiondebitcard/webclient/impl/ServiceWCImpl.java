package com.nttdata.bc19.mstransactiondebitcard.webclient.impl;
...
import com.nttdata.bc19.mstransactiondebitcard.webclient.IServiceWC;
...
public class ServiceWCImpl implements IServiceWC {
    ...
    @Override
    public Mono<DebitCardPerson> findDebitCardPersonById(String id) {
        return webClient.baseUrl(URL_GATEWAY).build().get().uri("/api/debit-card/person/".concat(id))
                .retrieve()
                .bodyToMono(DebitCardPerson.class)
                .onErrorResume(error -> {
                    WebClientResponseException response = (WebClientResponseException) error;
                    if (response.getStatusCode() == HttpStatus.NOT_FOUND) {
                        return Mono.error(new ModelNotFoundException("DEBIT CARD PERSON NOT FOUND"));
                    }
                    return Mono.error(error);
                });
    }
    ...
    @Override
    public Mono<DebitCardBusiness> findDebitCardBusinessById(String id) {
        return webClient.baseUrl(URL_GATEWAY).build().get().uri("/api/debit-card/business/".concat(id))
                .retrieve()
                .bodyToMono(DebitCardBusiness.class)
                .onErrorResume(error -> {
                    WebClientResponseException response = (WebClientResponseException) error;
                    if (response.getStatusCode() == HttpStatus.NOT_FOUND) {
                        return Mono.error(new ModelNotFoundException("DEBIT CARD BUSINESS NOT FOUND"));
                    }
                    return Mono.error(error);
                });
    }
